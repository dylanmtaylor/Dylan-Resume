...
sudo docker exec -it resume-builder bash -c 'apt-get update && export DEBIAN_FRONTEND=noninteractive && apt-get install -y texlive-xetex texlive-fonts-recommended texlive-font-utils'

sudo docker exec -it resume-builder bash -c 'cd /source &&  xelatex --output-directory=/destination dylan-resume.tex; ls -lah /destination/'
